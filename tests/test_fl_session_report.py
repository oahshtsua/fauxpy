...
def test_generate_report_statement_level():
    project_path = FauxpyPath.from_relative_path("/project", ".")
    scored_dict = {
        "technique1": [
    ...
    assert (
        "+0.7568" not in output
    )  # Should not have sign for positive score if all scores are non-negative
...
def test_get_rounded_scores():
    scored_dict = {"tech": [("file::10", 0.123456), ("file::20", 0.987654)]}
