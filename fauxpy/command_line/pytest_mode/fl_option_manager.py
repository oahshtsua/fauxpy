...
class FlOptionManager:
    ...
    @staticmethod
    def _get_validate_budget(budget_opt: Optional[str]) -> Optional[int]:
        """
        Validates the budget option.

        Args:
            budget_opt (str): The budget option, which should be greater than or equal to 0, or None.

        Returns:
            int: The validated budget value, or None if not provided.

        Raises:
            pytest.UsageError: If the option is not a valid integer or is less than zero.
        """
        # If budget_opt is None, return None (optional parameter)
        if budget_opt is None:
            return None

        error_message = (
            f"{budget_opt} is not a valid option. "
            f"Correct options: an integer x, where x >= 0, or omit to disable budget."
        )
        try:
